...
- Question-paper output is validated against a minimal page-observable contract
  (label, prompt, source pages, confidence and notes), then normalized into the
  full application draft. Solutions, rubrics and verification metadata are added
  later by backend logic. Cover/blank pages, administrative matter and reference
  sheets are excluded; independently answerable subquestions are separate items.
...
- Document responses reuse the existing content-addressed cache. Ordered pages,
  media type and an explicit versioned questions/solutions/student extraction
  contract contribute to document keys; unrelated application-model fields do
  not. Existing single-image cache keys are unchanged. Offline mode requires
  previously cached inputs.
...
- **20 MB / 20 pages** per PDF, with an additional 18 MB rendered-image budget.
  ...
  Question, solution and student extraction retry malformed structured output
  once with a corrective instruction. A second invalid result returns a safe
  error without saving a new or partially mutated import. Truncated structured
  responses follow the same bounded retry; use a smaller source document if both
  attempts exhaust the output limit.
